...
String urlFile) {
  public static MessageToResponse send(Message message){
    ...
    return new MessageToResponse(message.getMessage(),message.getFrom(),message.getTo(),formatter.format(message.getNow()),message.getUser(),message.getParticipants(),message.getUrlFile());
  }    
}
