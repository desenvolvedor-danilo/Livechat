...
$("#allow-button").click(() => {
    ...
    Notification.requestPermission().then((permission) => {
        if (permission === "granted") {
            fetch("/users/allow-notification",{method:"POST",headers:{"Content-Type":"application/json;charset=UTF-8"},body:JSON.stringify({"email":localStorage.getItem("email"),"notificated":true})}).then((res)=>console.log(res.text())).then(()=>hideDiv())
...
function updateLiveChat(message) {
    ...
    userCurrent = localStorage.getItem("usuario")
    ...
    if (userCurrent == message.from) {
        ...
        $("#livechat").append("<div class='msg-received'>" + message.content + "</div>")
    } else {
        //  $("#livechat").append("<p id='textodeoutro'>" + message.from + "</p>")
        $("#livechat").append("<div class='msg-sent'>" + message.content + "</div>")
    }
}
